...
function ModuleContent({ id, brief, recipe }: { id: BriefModuleId; brief: DecisionBriefV1; recipe: BriefRecipeV1 }) {
  ...
  switch (id) {
    ...
    case "scope": { const scope = brief.movable.scope.value; return <div><div className={styles.metric}><div className={styles.label}>Canonical executable work</div><div className={styles.metricValue}>{scope.executableItemCount} items</div><div className={styles.metricSub}>{n(scope.remainingEffortDays.low)} / {n(scope.remainingEffortDays.likely)} / {n(scope.remainingEffortDays.high)} effort days · <Link href={scope.href}>Open Scope</Link></div></div>{scope.capabilityOutlooks?.length ? <div className={styles.list} style={{ marginTop: 10 }}>{scope.capabilityOutlooks.map((outlook) => <div className={styles.listItem} key={outlook.capabilityId}><span><strong>{outlook.name}</strong><small style={{ display: "block" }}>{outlook.contributors.map((person) => `${person.name} ${n(person.fte)} FTE`).join(" · ")} · isolated</small></span><strong>{date(outlook.likelyDate)}</strong></div>)}</div> : null}</div>; }
    ...
    case "source-health": return <ul className={styles.list}>{brief.identity.sourceSnapshots.map((source, index) => <li className={styles.listItem} key={`${source.owner}-${index}`}><span>{source.owner}</span><strong>{source.temporalRole} · {source.currentness} · {instantDate(source.asOf)}</strong></li>)}</ul>;

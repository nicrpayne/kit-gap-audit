...
function ModuleContent({ id, brief, recipe }: { id: BriefModuleId; brief: DecisionBriefV1; recipe: BriefRecipeV1 }) {
  ...
  switch (id) {
    ...
    case "timeline": return <div><div className={styles.metric} style={{ "--tone": brief.timeline.currentForecast.source.currentness === "stale" ? "#e5b84b" : "#7d9eff" } as React.CSSProperties}><div className={styles.label}>Live Forecast · {brief.timeline.currentForecast.source.currentness} · as of {date(brief.timeline.currentForecast.source.asOf)}</div><div className={styles.metricValue}>{date(brief.timeline.currentForecast.value.likelyDate)}</div><div className={styles.metricSub}><Link href={brief.timeline.currentForecast.value.href}>Open Forecast</Link></div></div><p className={styles.copy} style={{ marginTop: 8 }}>Next milestone: {brief.timeline.nextMilestone.value ? `${brief.timeline.nextMilestone.value.title} · ${date(brief.timeline.nextMilestone.value.date)}` : "MISSING"}</p></div>;

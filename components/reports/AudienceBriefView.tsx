...
function Stamp({ source }: { source: SourceStamp }) {
  return <div className={styles.stamp}>{source.owner} · {source.temporalRole} · as of {instantDate(source.asOf)} · {source.currentness}</div>;
}
...
function ModuleContent({ id, brief, recipe }: { id: BriefModuleId; brief: DecisionBriefV1; recipe: BriefRecipeV1 }) {
  ...
  switch (id) {
    ...
    case "scope": { const scope = brief.movable.scope.value; const quality = scope.estimateQuality; return <div><div className={styles.metric}><div className={styles.label}>Canonical executable work</div><div className={styles.metricValue}>{scope.executableItemCount} items</div><div className={styles.metricSub}>{n(scope.remainingEffortDays.low)} / {n(scope.remainingEffortDays.likely)} / {n(scope.remainingEffortDays.high)} effort days · <Link href={scope.href}>Open Scope</Link></div>{quality && <div className={styles.metricSub} style={{ marginTop: 6 }}>Estimate quality · {quality.pointsIssueCount} Linear · {quality.aiCount} AI · {quality.placeholderIssueCount + quality.placeholderFindingCount} placeholders · {quality.placeholderEffortSharePct}% placeholder effort</div>}</div>{scope.capabilityOutlooks?.length ? <div className={styles.list} style={{ marginTop: 10 }}>{scope.capabilityOutlooks.map((outlook) => <div className={styles.listItem} key={outlook.capabilityId}><span><strong>{outlook.name}</strong><small style={{ display: "block" }}>{outlook.contributors.map((person) => `${person.name} ${n(person.fte)} FTE`).join(" · ")} · isolated</small></span><strong>{date(outlook.likelyDate)}</strong></div>)}</div> : null}</div>; }
    ...
    case "timeline": return <div><div className={styles.metric} style={{ "--tone": brief.timeline.currentForecast.source.currentness === "stale" ? "#e5b84b" : "#7d9eff" } as React.CSSProperties}><div className={styles.label}>Live Forecast · {brief.timeline.currentForecast.source.currentness} · as of {instantDate(brief.timeline.currentForecast.source.asOf)}</div><div className={styles.metricValue}>{date(brief.timeline.currentForecast.value.likelyDate)}</div><div className={styles.metricSub}><Link href={brief.timeline.currentForecast.value.href}>Open Forecast</Link></div></div><p className={styles.copy} style={{ marginTop: 8 }}>Next milestone: {brief.timeline.nextMilestone.value ? `${brief.timeline.nextMilestone.value.title} · ${date(brief.timeline.nextMilestone.value.date)}` : "MISSING"}</p></div>;
    ...
    case "source-health": return <ul className={styles.list}>{brief.identity.sourceSnapshots.map((source, index) => <li className={styles.listItem} key={`${source.owner}-${index}`}><span>{source.owner}{source.note && <small style={{ display: "block" }}>{source.note}</small>}</span><strong>{source.temporalRole} · {source.currentness} · {instantDate(source.asOf)}</strong></li>)}</ul>;

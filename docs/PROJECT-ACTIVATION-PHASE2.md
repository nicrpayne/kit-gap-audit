...
The operational producer now lives in the standalone local `kit-gap-bridge` repository. Version `0.2.0` at commit `d227238a2cb95d4c2ccbae6b8e61f6e20e167543` selects from canonical project identity and aliases before a Scope exists, takes currency only from `current-state.json`'s `is_head`, follows bounded typed graph edges, resolves linked Evidence Passages, distinguishes raw-source lineage from derivative wiki context, and produces a deterministic v1.1 package. `kit-gap bootstrap` creates or resolves the dormant ProjectBootstrap, validates and materializes the package locally, and performs one authenticated package POST. Railway receives only the bounded package; it never receives or assumes a path into the local corpus.

The bridge repository currently has no Git remote. Its commit is a clean local integration, not yet a governed distributable. A reviewed remote, immutable release tag/artifact, installation procedure, and operator upgrade/rollback policy are required before production use. Signal does not carry a second operational producer implementation.
